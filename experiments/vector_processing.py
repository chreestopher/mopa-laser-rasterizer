...
# Experiment-only pipeline behavior. Keep the production synthetic-canvas
# geometry, but organize output into spatial exterior-region groups.
NO_SYNTHETIC_BLACK_CANVAS = False
SPATIALLY_GROUP_PUNCHTHROUGH = True
CONSTRAIN_NONBLACK_DEFAULT = False
...
def load_resized_source_black_cutoff_mask(raster_image_path, output_size):
    """Resize the source cutoff mask without interpolation changing membership."""
...
def restore_reserved_black(quantized_img, source_black_mask):
    """Restore exact source Black after quantization to a non-Black palette."""
...
def _mask_to_merged_geometry(mask):
    """Convert a boolean raster mask into exact coalesced rectangle geometry."""
...
def replace_krasnow_black_layer(processed_layers, black_hex, source_img):
    """Replace only Black with the Lab mask, preserving every carrier layer."""
...
def prepare_raster_image(
    ...
    if quantize_colors is not None:
        ...
        if active_swatches:
            ...
            # A Pillow palette always has 256 entries. Padding unused entries
            # with zero silently introduces Black as a candidate even when
            # Black was deliberately excluded. Repeat the final valid swatch
            # so every physical palette slot remains non-Black.
            final_swatch = palette_values[-3:]
            padding_entries = 256 - len(active_swatches[:256])
            palette.putpalette(
                palette_values + final_swatch * padding_entries
            )
            img = img.quantize(
...
def _export_geometry_parts(geometry):
    if geometry is None or geometry.is_empty:
        return []
    if geometry.geom_type in ("Polygon", "LineString"):
        return [geometry]
    if hasattr(geometry, "geoms"):
        ...
        for item in geometry.geoms:
            parts.extend(_export_geometry_parts(item))
        return parts
    return []


def build_spatial_export_groups(processed_layers, black_hex):
    """Group shapes under the outermost non-Black polygon containing them."""
    records = []
    nonblack_polygon_indices = []
    polygons = []
    for color_hex, geometry in processed_layers.items():
        for part in _export_geometry_parts(geometry):
            record_index = len(records)
            records.append({"color": color_hex, "geometry": part})
            if color_hex != black_hex and part.geom_type == "Polygon":
                nonblack_polygon_indices.append(record_index)
                # Containment is based on the exterior ring, intentionally
                # including holes where interior artwork can live.
                polygons.append(Polygon(part.exterior))
    ...
    polygon_to_record = {
        polygon_index: record_index
        for polygon_index, record_index in enumerate(nonblack_polygon_indices)
    }

    def containing_polygon_indices(geometry):
        point = geometry.representative_point()
        try:
            return [int(index) for index in tree.query(point, predicate="within")]
        except TypeError:
            # Compatibility with Shapely versions whose STRtree returns
            # geometries and does not accept a predicate.
            candidates = tree.query(point)
            return [
                index for index, candidate in enumerate(polygons)
                if candidate in candidates and point.within(candidate)
            ]
    ...
    for record_index, record in enumerate(records):
        ...
        if geometry.geom_type != "Polygon":
            continue
        candidates = []
        for polygon_index in containing_polygon_indices(geometry):
            candidate_record = polygon_to_record[polygon_index]
            candidate = polygons[polygon_index]
            if candidate_record == record_index:
                continue
            if candidate.area > geometry.area and candidate.covers(
                geometry.representative_point()
            ):
                candidates.append(candidate_record)
        if candidates:
            parent_by_record[record_index] = min(
                candidates, key=lambda index: records[index]["geometry"].area
            )

    def outermost_parent(record_index):
        seen = set()
        current = record_index
        while current in parent_by_record and current not in seen:
            seen.add(current)
            current = parent_by_record[current]
        return current

    groups = defaultdict(list)
    for record_index, record in enumerate(records):
        if record["color"] == black_hex:
            group_root = "black-layer"
        elif record["geometry"].geom_type == "Polygon":
            candidates = containing_polygon_indices(record["geometry"])
            group_root = outermost_parent(record_index)
        else:
            group_root = record_index
        groups[group_root].append(record)

    return list(groups.values())


def export_spatially_grouped_layers(
    processed_layers,
    target_colors,
    black_hex,
    scale_factor,
    ...
    lb_project_instance,
    punch_through_black,
    black_lightburn_geometry,
    export_lightburn,
):
    """Export one SVG and LightBurn group per containing exterior region."""
    scaled_layers = {
        color: (
            scale(geometry, xfact=scale_factor, yfact=scale_factor, origin=(0, 0))
            if scale_factor != 1.0 else geometry
        )
        for color, geometry in processed_layers.items()
    }
    groups = build_spatial_export_groups(scaled_layers, black_hex)
    artwork_group = ET.SubElement(root, "g", id="rasterized-artwork")

    if export_lightburn and black_lightburn_geometry is not None:
        black_canvas = black_lightburn_geometry
        if scale_factor != 1.0:
            black_canvas = scale(
                black_canvas, xfact=scale_factor, yfact=scale_factor, origin=(0, 0)
            )
        lb_project_instance.startgroup()
        push_geometry_to_lightburn(
            black_canvas, black_hex, target_colors, lb_project_instance,
            override_layer_id=target_colors[black_hex][1],
        )
        if punch_through_black:
            for color_hex, geometry in scaled_layers.items():
                if color_hex == black_hex or geometry.is_empty:
                    continue
                push_geometry_to_lightburn(
                    geometry, color_hex, target_colors, lb_project_instance,
                    override_layer_id=target_colors[black_hex][1],
                )
        lb_project_instance.endgroup()

    for group_index, records in enumerate(groups, 1):
        black_only_group = all(
            record["color"] == black_hex for record in records
        )
        svg_group = ET.SubElement(
            artwork_group,
            "g",
            id=("black-layer-group" if black_only_group else f"spatial-region-{group_index}"),
        )
        if export_lightburn:
            lb_project_instance.startgroup()
        for record in records:
            color_hex = record["color"]
            geometry = record["geometry"]
            add_geometry_to_svg(svg_group, geometry, color_hex)
            if not export_lightburn or color_hex not in target_colors:
                continue
            if color_hex != black_hex or black_lightburn_geometry is None:
                push_geometry_to_lightburn(
                    geometry, color_hex, target_colors, lb_project_instance
                )
        if export_lightburn:
    ...
    printLogMessage(
        f"Spatial grouping exported {len(groups)} exterior-region groups."
    )
...
def export_processed_layers(
    ...
    if SPATIALLY_GROUP_PUNCHTHROUGH:
        return export_spatially_grouped_layers(
            ...
            punch_through_black=punch_through_black,
...
def raster_to_puzzle_and_lightburn(
    ...
    In this experimental copy, Black is ordinary source-derived geometry.
    No synthetic canvas is created and no colored paths are duplicated onto
    the Black LightBurn layer for punch-through.
    ...
    constrain_nonblack = bool(
        (filter_parameters or {}).get(
            "constrain_nonblack", CONSTRAIN_NONBLACK_DEFAULT
        )
    )
    printLogMessage(
        "Constrain Nonblack: "
        + (
            "enabled; source pixels darker than LightBurn Teal (#004754) are reserved for Black."
            if constrain_nonblack
            else "disabled; ordinary quantization may assign dark pixels to Black."
        )
    )
    reserve_exact_black = (
        constrain_nonblack
        and image_preset != "bw_dither_photograph"
    )
    source_black_mask = None
    img = prepare_raster_image(
        ...
        target_colors=(None if image_preset == "bw_dither_photograph" else {
            ...
            if (
                ...
                and (not reserve_exact_black or color_hex != black_hex)
            )
        })
    )

    if reserve_exact_black:
        ...
        quantized_black_mask = np.all(
            np.asarray(img.convert("RGB"), dtype=np.uint8) == 0, axis=2
        )
        unexpected_black = quantized_black_mask & ~source_black_mask
        if np.any(unexpected_black):
            raise RuntimeError(
                "Constrain Nonblack invariant failed: quantization introduced "
                f"{int(np.count_nonzero(unexpected_black))} unexpected Black pixels."
            )
        printLogMessage(
            f"Reserved {int(np.count_nonzero(source_black_mask))} pixels sampled "
            "from a below-Teal-luminance original-source mask; all other pixels were "
            "quantized without Black as a candidate."
        )
    ...
    if centerline_mode:
        ...
        pixel_boxes_by_color = classify_raster_pixels(
            ...
            black_hex=black_hex,
            ignore_background_hex=ignore_background_hex,
            include_black=(NO_SYNTHETIC_BLACK_CANVAS or preserve_source_black),
    ...
    if filter_name == "krasnow_grating":
        # Krasnow's carrier geometry above is intentionally left untouched.
        # Replace only its final Black layer with the same independent,
        # adaptive two-color dark mask used by the Holographic Etching Lab.
        black_source_image = filter_parameters.get("_angle_image")
        if black_source_image is None:
            black_source_image = prepare_raster_image(
                raster_image_path=raster_image_path,
                new_height=new_height,
                new_width=new_width,
                quantize_colors=None,
        ...
        printLogMessage(
            "Krasnow Color Grating: replaced only the Black layer with the "
            "Holographic Etching Lab adaptive dark mask; carrier layers unchanged."
        )
    ...
    black_lightburn_geometry = None
    if NO_SYNTHETIC_BLACK_CANVAS:
        printLogMessage(
            "No-canvas experiment: exporting source-derived Black geometry "
            "without synthetic canvas or punch-through."
        )
    elif not preserve_source_black:
    ...
    export_processed_layers(
        ...
        lb_project_instance=lb_project_instance,
        punch_through_black=(
            not NO_SYNTHETIC_BLACK_CANVAS and not preserve_source_black
        ),
        black_lightburn_geometry=black_lightburn_geometry,
        export_lightburn=export_lightburn,
    )
