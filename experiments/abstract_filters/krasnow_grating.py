...
Black is classified from the source raster, then transformed through the same
patch-and-grating remap as every other input color. This preset does not
synthesize a Black canvas or punch into Black. The selected material's
Holographic recipe is treated as a calibrated 1-micron anchor. Every non-black
output carrier layer receives an independent copy whose speed is scaled to
create its target microscopic pitch while retaining the anchor's other values.
...
# Keep Black in the source buckets and suppress the generic synthetic canvas.
# remap_layers still transforms Black through the grating carriers below.
...
# The general Abstract preset uses aggressive cleanup intended for broad
# filled shapes. A 5-pixel morphological close merges nearby Black outlines
# into blobs before this filter can preserve them. Krasnow needs source-faithful
# regions before it constructs its grating paths.
VECTOR_DEFAULTS = {
    "min_island_area": 0,
...
def remap_layers(processed_layers, target_colors, settings):
    """Build open gratings from every source layer, including Black."""
