...
DOCS["krasnow-grating-filter"].update({
    ...
    "sections": [
        ...
        ("Pitch extrapolation", ["Pulse pitch is proportional to speed divided by frequency. With frequency fixed, each target layer uses: layer speed = Holographic anchor speed × target pitch / 1.00 µm. Available layers are spread in LightBurn order from approximately 0.55 to 1.55 µm; a single non-Black layer remains at the 1.00 µm anchor.", "For a 300 mm/s Holographic anchor, a full 21-level progression becomes 165, 180, 195, and so on through 465 mm/s at 300 kHz. This follows the dominant fixed-frequency speed progression in Ben Krasnow's published LightBurn example while adapting it to any available layer count."]),
