...
DOCS["krasnow-grating-filter"].update({
    "description": "Generate Ben Krasnow-style open-path grating patches whose non-Black output layers each clone the selected material's Holographic laser setting.",
    ...
    "sections": [
        ...
        ("Holographic layer settings", ["The selected material must contain an entry whose description or setting name matches Holographic. That entry is loaded once as a template, then independently cloned onto every non-Black output layer that survived normal palette and Material Library matching.", "Each clone retains its output layer's original LightBurn ID, color, and name, but receives the Holographic entry's power, speed, frequency, pulse width, passes, and other laser values. Offset Fill entries use their first concrete sublayer. The operation is forced to Line/Cut because the filter has already generated open vector paths. The temporary template layer is not exported."]),
        ...
        ("Calibration", ["There is no fixed swatch or layer count. Each available non-black color remains a separate grating-level carrier, but all carriers use clones of the same Holographic laser recipe.", "Layer colors remain organizational identifiers rather than guaranteed engraved colors. Because every clone has the same speed and frequency, this setting replacement does not by itself create different microscopic pulse pitches for different colors. True Black retains only the geometry and setting associated with Black in the source workflow."]),
