...
DOCS["krasnow-grating-filter"].update({
    "description": "Generate Ben Krasnow-style open-path grating patches with hue-driven layers and a vertical viewing-angle correction while preserving only source-derived Black geometry.",
    ...
    "sections": [
        ...
        ("Calibration", ["There is no fixed swatch or layer count. Each selected non-black layer is both an available quantization color and a grating-level carrier, so the filter cannot generate a layer that the normal pipeline did not load.", "Layer colors remain identifiers rather than guaranteed engraved colors. Arrange their Material Library recipes in increasing grating-level order; the filter does not invent safe speed, frequency, power, pulse width, or line spacing settings. True Black retains only the geometry found in the source raster."]),
