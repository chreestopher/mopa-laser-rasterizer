...
DOCS["krasnow-grating-filter"].update({
    ...
    "sections": [
        ("What remains unchanged", ["The source image is resized against the selected, material-backed palette, quantized to those available colors, and cleaned into closed regions by the normal pipeline. Those closed regions become the Black punch mask, while the visible grating output is rebuilt as open paths. No separate palette is loaded, and other filters and image presets do not use these line-generation and punch-mask capabilities."]),
        ("Open-path geometry", ["Each local patch is filled with parallel LineString paths clipped to the prepared artwork. The defaults match Ben's 0.4 mm patch size and 0.06 mm line spacing. The source luminance maps from -90 to +90 degrees by default, matching his grayscale-angle convention.", "Every available non-black output layer is exported in LightBurn Line mode. The grating paths are open lines, and the closed artwork geometry is used only for Black punch-through."]),
