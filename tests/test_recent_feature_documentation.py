...
class RecentFeatureDocumentationTests(unittest.TestCase):
    def test_recent_workflow_guides_render_and_are_linked(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertIn("Independent random rotation", geometry)
            self.assertIn("same Seed and job settings reproduce", geometry)
            ...
            self.assertIn("former Abstract Image Style was retired", krasnow)
            self.assertIn("legacy filter has been removed", krasnow)
            self.assertIn("Nothing from the former baseline workflow is lost", krasnow)
            self.assertIn("assign every eligible swatch to Krasnow", krasnow)
            self.assertIn("Compared with the retired filter", krasnow)
            self.assertIn("experimental LightBurn Fill rendering", krasnow)
            ...
            self.assertIn("Custom accepts an uploaded SVG", krasnow)
            self.assertIn("Random Rotation gives individual cells", krasnow)
