...
def apply_source_black(geometries, settings):
    """Apply one indexed Deep Fryer field to hole-free Black components."""
        ...
        while x < x2:
            ...
            if not tile.is_empty:
                ...
                candidate_indexes = sorted(
                    int(index)
                    for index in source_index.query(tile, predicate="intersects")
                )
                for candidate_index in candidate_indexes:
                    fragment_parts.extend(_polygonal_parts(
                        geometries[candidate_index].intersection(tile)
                    ))
                ...
                if fragment_parts:
                    ...
                    if _noise(index + 31) >= degradation * .55:
                        local_offset = (
                            (_noise(index + 67) - .5) * smear_amount * .42
                        )
                        direction = -1 if _noise(index + 101) < .5 else 1
                        for part in fragment_parts:
                            shifted = translate(
                                part, xoff=band_offset + local_offset
                            ).intersection(canvas)
                            pieces.extend(_polygonal_parts(shifted))
                            for echo in range(1, echo_count + 1):
                                if (
                                    _noise(index + echo * 179)
                                    < .35 + degradation * .55
                                ):
                                    echo_piece = translate(
                                        part,
                                        xoff=(
                                            band_offset
                                            + local_offset
                                            + direction * echo * echo_spacing
                                        ),
                                    ).intersection(canvas)
                                    pieces.extend(_polygonal_parts(echo_piece))
            x += block_size
            column += 1
        y += band_height
        row += 1
